...
export default function ClientGroupsPage() {
  ...
  const [form, setForm] = useState({ name: '', discount: 0, description: '' });

  const load = async () => {
    setLoading(true);
    try {
      const data = await api.request('/api/client-groups');
      setItems(data);
  ...
  const openAdd = () => {
    setEditId(null);
    setForm({ name: '', discount: 0, description: '' });
    setShowModal(true);
  };

  const openEdit = (item: any) => {
    setEditId(item.id);
    setForm({ name: item.name, discount: item.discount || 0, description: item.description || '' });
    setShowModal(true);
  };
  ...
  const save = async () => {
    ...
    try {
      if (editId) await api.request(`/api/client-groups/${editId}`, { method: 'PUT', body: JSON.stringify(form) });
      else await api.request('/api/client-groups', { method: 'POST', body: JSON.stringify(form) });
      setShowModal(false);
      load();
  ...
  const remove = async (id: number) => {
    if (!confirm('Удалить группу клиентов?')) return;
    try {
      await api.request(`/api/client-groups/${id}`, { method: 'DELETE' });
      load();
    } catch (e: any) { addToast(e.message, 'error'); }
  };
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-white">Группы клиентов</h1>
          <p className="text-sm text-zinc-500">Управление группами клиентов и скидками</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 dark:border-zinc-700">
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">Скидка</th>
            ...
            <tbody>
              {items.map((item: any) => (
                <tr key={item.id} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                  ...
                  <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{item.discount}%</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={4} className="text-center py-12 text-zinc-400">Нет групп клиентов</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-zinc-500">Скидка (%)</label>
