...
export default function MenuModifierGroupsPage() {
  ...
  const [form, setForm] = useState({ name: '', description: '', min_count: 0, max_count: 0 });

  const load = async () => {
    setLoading(true);
    try {
      const data = await api.request('/api/modifier-groups');
      setItems(data);
  ...
  const openAdd = () => {
    setEditId(null);
    setForm({ name: '', description: '', min_count: 0, max_count: 0 });
    setShowModal(true);
  };
  ...
  const openEdit = (item: any) => {
    ...
    setForm({
      ...
      min_count: item.minCount ?? item.min_count ?? 0,
      max_count: item.maxCount ?? item.max_count ?? 0,
    });
    setShowModal(true);
  };

  const save = async () => {
    if (!form.name.trim()) return addToast('Введите название группы', 'error');
    try {
      const payload = {
        name: form.name,
      ...
      if (editId) await api.request(`/api/modifier-groups/${editId}`, { method: 'PUT', body: JSON.stringify(payload) });
      else await api.request('/api/modifier-groups', { method: 'POST', body: JSON.stringify(payload) });
      setShowModal(false);
      load();
  ...
  const remove = async (id: number) => {
    ...
    try {
      await api.request(`/api/modifier-groups/${id}`, { method: 'DELETE' });
      load();
    } catch (e: any) { addToast(e.message, 'error'); }
  };
    ...
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 bg-violet-100 dark:bg-violet-900/30 rounded-xl flex items-center justify-center">
          <FolderTree size={22} className="text-violet-600 dark:text-violet-400" />
        </div>
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-white">Группы модификаторов</h1>
          <p className="text-sm text-zinc-500">Управление группами модификаторов</p>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-zinc-200 dark:border-zinc-700">
                ...
                <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">Описание</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">Мин.</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-zinc-500 uppercase tracking-wider">Макс.</th>
            ...
            <tbody>
              {items.map((item: any) => (
                <tr key={item.id} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                  ...
                  <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400 max-w-xs truncate">{item.description || '—'}</td>
                  <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{item.minCount ?? item.min_count ?? 0}</td>
                  <td className="px-4 py-3 text-zinc-600 dark:text-zinc-400">{item.maxCount ?? item.max_count ?? 0}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={5} className="text-center py-12 text-zinc-400">Нет групп модификаторов</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">
                {editId ? 'Редактировать группу' : 'Добавить группу'}
              </h3>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-zinc-500">Минимум</label>
                  <input type="number" value={form.min_count} onChange={e => setForm({...form, min_count: Number(e.target.value)})}
                    className="w-full border-2 border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white mt-1" />
                </div>
                <div>
                  <label className="text-xs font-medium text-zinc-500">Максимум</label>
                  <input type="number" value={form.max_count} onChange={e => setForm({...form, max_count: Number(e.target.value)})}
                    className="w-full border-2 border-zinc-200 dark:border-zinc-700 rounded-xl px-4 py-2.5 text-sm bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white mt-1" />
                </div>
              </div>
              <button onClick={save}
