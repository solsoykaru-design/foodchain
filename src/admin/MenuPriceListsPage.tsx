...
export default function MenuPriceListsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await api.request('/api/wholesale-prices');
      setItems(data);
  ...
  const save = async () => {
    if (!form.name.trim()) return addToast('Введите название прайс-листа', 'error');
    try {
      if (editId) await api.request(`/api/wholesale-prices/${editId}`, { method: 'PUT', body: JSON.stringify(form) });
      else await api.request('/api/wholesale-prices', { method: 'POST', body: JSON.stringify(form) });
      setShowModal(false);
      load();
  ...
  const remove = async (id: number) => {
    ...
    try {
      await api.request(`/api/wholesale-prices/${id}`, { method: 'DELETE' });
      load();
    } catch (e: any) { addToast(e.message, 'error'); }
  };
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-white">Прайс-листы</h1>
          <p className="text-sm text-zinc-500">Управление прайс-листами меню</p>
        </div>
      </div>
      ...
      <button onClick={openAdd} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-blue-700 active:scale-[0.97] transition-all mb-5">
        <Plus size={18} /> Добавить прайс-лист
      </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {items.length === 0 && (
                <tr><td colSpan={4} className="text-center py-12 text-zinc-400">Нет прайс-листов</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-3xl p-6 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-zinc-900 dark:text-white">
                {editId ? 'Редактировать прайс-лист' : 'Добавить прайс-лист'}
              </h3>
