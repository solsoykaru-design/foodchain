...
const columns: ColumnDef[] = [
  { key: 'day', label: 'День', format: 'date' },
  ...
  { key: 'guests', label: 'Гости', format: 'number', align: 'right' },
];

export default function SalesSummary({ from, to, branchId }: { from: string; to: string; branchId?: number }) {
  ...
  useEffect(() => {
    setLoading(true);
    api.getReportSalesSummary({ from, to, branch_id: branchId })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-zinc-900 dark:text-white">Сводка продаж</h2>
        <ExportButton data={data} columns={columns} filename="sales-summary" title="Сводка продаж" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-4 shadow-sm border border-zinc-200 dark:border-zinc-800">
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Гости</p>
          <p className="text-2xl font-bold text-zinc-900 dark:text-white">{(totals.guests ?? 0).toLocaleString()}</p>
        </div>
      </div>
      <ReportChart type="line" data={data} xKey="day" yKey="revenue" yKey2="avgCheck" title="Динамика выручки и среднего чека" />
      <ReportTable columns={columns} data={data} summary={{ day: 'Итого', revenue: totals.revenue, orders: totals.orders, avgCheck: totals.avgCheck, guests: totals.guests }} />
    </div>
  );
}
