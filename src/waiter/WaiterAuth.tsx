...
import { UserCircle, Lock, LogIn, Store } from 'lucide-react';
import * as api from '../api';

export default function WaiterAuth({ onLogin }: { onLogin: (user: any) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.user || !(res.user.role === 'waiter' || res.user.role === 'manager' || res.user.role === 'superadmin')) {
        setError('У вас нет прав доступа к терминалу официанта');
        setLoading(false);
        return;
      }
      localStorage.setItem('foodchain_waiter_token', res.token || '');
      localStorage.setItem('foodchain_last_tenant', tenantName.trim());
      localStorage.setItem('foodchain_last_login', username);
      onLogin({ ...res.user, tenant: { id: res.user.tenantId, name: res.user.tenantName || tenantName } });
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-gradient-to-br from-orange-500 to-red-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-orange-500/20">
            <UserCircle size={32} className="text-white" />
          </div>
          <h1 className="text-2xl font-extrabold text-white">Терминал официанта</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-zinc-900 rounded-xl ring-1 ring-zinc-800 flex items-center px-4">
            <UserCircle size={20} className="text-zinc-500 mr-3" />
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-orange-500 text-white font-bold py-3.5 rounded-xl active:scale-[0.99] transition-transform flex items-center justify-center gap-2 disabled:opacity-60">
