...
app.get('/api/telegram-bot/settings', (req, res) => {
  try { res.json(telegramBot.getSettings(db, req.tenant_id || 1)); }
  catch (e) { res.status(500).json({ error: safeError(e.message) }); }
});
app.put('/api/telegram-bot/settings', (req, res) => {
  try { res.json(telegramBot.saveSettings(db, req.body, req.tenant_id || 1)); }
  catch (e) { res.status(500).json({ error: safeError(e.message) }); }
});
app.post('/api/telegram-bot/restart', (req, res) => {
  try {
    ...
    telegramBot.stopBot();
    telegramBot.startIfConfigured(db, req.tenant_id || 1);
...
app.get('/api/telegram-bot/stats', (req, res) => {
  try { res.json(telegramBot.getStats(db, req.tenant_id || 1)); }
  catch (e) { res.status(500).json({ error: safeError(e.message) }); }
});
app.post('/api/telegram-bot/broadcast', (req, res) => {
  try {
    const settings = telegramBot.getSettings(db, req.tenant_id || 1);
...
app.post('/api/telegram-bot/subscribe', (req, res) => {
  ...
  db.prepare("INSERT OR IGNORE INTO telegram_order_subscriptions (tenant_id, chat_id, order_id) VALUES (?, ?, ?)").run(req.tenant_id || 1, chat_id, order_id);
  res.json({ success: true });
});
